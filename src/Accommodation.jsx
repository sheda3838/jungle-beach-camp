...
const INFO_CARDS = [
  {
    ...
    items: [
      'Nearest train: Unawatuna (2 km)',
      'Bus routes via A2 highway',
      'Easy access from Galle',
    ],
  },
  {
    id: 2,
    ...
    title: 'Safety Notes',
    items: [
      'Watch sharp coral areas',
      'Swim with caution near reef',
      'Respect marine life',
    ],
  },
  {
    ...
    items: [
      'Seasonal DJ nights',
      'Beach café music events',
      'Informal sunset gatherings',
    ],
  },
];
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 p-5 lg:p-6">
        <h3
          className="font-serif text-2xl lg:text-3xl font-bold text-[#f5ead4] leading-tight mb-3 drop-shadow-lg"
        >
          {card.title}
        </h3>
        <ul className="space-y-1.5">
          {card.points.map((pt, i) => (
            <li key={i} className="flex items-start gap-2.5">
              ...
              />
              <span className="font-sans text-[13px] text-[#f5ead4]/80 leading-snug font-light">
                {pt}
              </span>
            </li>
          ))}
    ...
    >
      ...
      <div className="relative p-4 lg:p-5 h-full flex flex-col gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <h4 className="font-serif text-base font-semibold text-[#f5ead4] leading-tight">
            {card.title}
          </h4>
        </div>
        ...
        <ul className="space-y-1.5 flex-1">
          {card.items.map((item, i) => (
            <li key={i} className="flex items-start gap-2">
              ...
              <span className="font-sans text-[12px] text-[#f5ead4]/65 leading-snug font-light">
                {item}
              </span>
            </li>
          ))}
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full h-full flex flex-col max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 xl:px-14 py-6 lg:py-8 gap-4 lg:gap-5">
        ...
        {/* ── Top: 3 Story Cards — ~65% of remaining space ── */}
        ...
        {/* ── Bottom: 3 Info Cards — ~35% of remaining space ── */}
      ...
      {/* ── Mobile scroll note (only on very small screens when layout stacks) ── */}
      <style>{`
